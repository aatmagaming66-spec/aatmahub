      ...
      <div className="p-6 space-y-6">
        ...
        <section className="space-y-4">
          ...
          <Card className="bg-card border-border rounded-3xl overflow-hidden shadow-2xl">
            <CardContent className="p-0">
              {[
                { icon: User, label: 'Edit Identity', action: () => setEditing(!editing) },
                { icon: Bell, label: 'Alert Protocols', action: () => {} },
                { icon: Lock, label: 'Encryption Key', action: () => {} }
              ].map((opt, i) => (
        ...
        {editing && (
          <Card className="bg-card border-border rounded-3xl p-6 space-y-4 animate-in slide-in-from-top-4">
            <div className="flex justify-between items-center"><h4 className="text-xs font-black uppercase text-primary">Update Protocol</h4><Button variant="ghost" className="text-[9px] uppercase" onClick={() => setEditing(false)}>Cancel</Button></div>
            <div className="space-y-4">
              ...
              <Button onClick={async () => { if (!user) return; setSaving(true); try { await updateDoc(doc(db, 'users', user.uid), { fullName, phoneNumber }); setEditing(false); toast({ title: 'Identity Synced' }); } finally { setSaving(false); } }} disabled={saving} className="w-full bg-primary font-black uppercase text-[10px] h-12 rounded-xl">{saving ? <Loader2 className="animate-spin" /> : 'Commit Changes'}</Button>
            </div>
          </Card>
        )}
