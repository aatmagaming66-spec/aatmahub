...
export default function CheckoutPage() {
    ...
    if (savedVerification) {
      try {
        ...
        if (data.isVerified && data.playerId && data.serverId) {
          console.log('[Checkout Audit] Loading verified profile:', data.playerId);
          setVerificationData(data);
        } else {
          console.warn('[Checkout Audit] Incomplete verification detected. Redirecting.');
          router.push('/');
        }
      } catch (e) {
        console.error('[Checkout Audit] Failed to parse verification.', e);
        router.push('/');
      }
    } else {
      router.push('/');
  ...
  if (userLoading || walletLoading || !user || !verificationData) {
  ...
  return (
    <div className="flex flex-col w-full p-4 space-y-10 animate-in fade-in duration-700">
      <header className="py-4">
        <h1 className="text-3xl font-headline font-black tracking-tighter uppercase">Order Confirmation</h1>
        <p className="text-[10px] text-muted-foreground uppercase tracking-[0.3em] font-black opacity-60">HUB-TO-SERVER PROTOCOL</p>
      </header>
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xs font-black uppercase tracking-widest text-white/80">Order Protocol Recap</h3>
        </div>
        <Card className="bg-card border-border rounded-3xl overflow-hidden shadow-2xl relative">
          ...
          <CardContent className="p-8 space-y-8">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <span className="text-[9px] font-black text-green-500 uppercase tracking-widest leading-none flex items-center gap-1.5">
                  <CheckCircle2 size={10} /> Verified Target Profile
                </span>
            ...
            <div className="grid grid-cols-2 gap-8 pt-6 border-t border-border">
              <div className="space-y-1.5">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                  <Package size={10} className="text-primary" /> Item / Quantity
                </span>
                <p className="text-sm font-black text-white uppercase truncate">{firstItem.name} <span className="text-primary">x{firstItem.quantity}</span></p>
              </div>
              <div className="space-y-1.5">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
            ...
            <div className="pt-8 border-t border-border flex justify-between items-end">
              <span className="text-xs font-black text-muted-foreground uppercase tracking-widest">Grand Total Amount</span>
          ...
          <Label htmlFor="phonepe" className={`flex items-center justify-between p-6 rounded-3xl border transition-all cursor-pointer bg-card shadow-xl ${paymentMethod === 'phonepe' ? 'border-primary ring-1 ring-primary/20' : 'border-border'}`}>
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Instant External Gateway</span>
              </div>
            </div>
      ...
      <div className="flex flex-col gap-4 pt-4">
        <Button 
          ...
          disabled={submitting}
        >
          {submitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Execute Order & Pay <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" /></>}
        </Button>
