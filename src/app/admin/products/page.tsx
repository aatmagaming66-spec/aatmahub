...
export default function AdminProductManagementPage() {
  ...
  const handleOpenRankModal = (rank: any = null) => {
    ...
    else { setEditingRank(null); setRankFormData({ id: '', name: '', threshold: 0, discount: 0, color: '#DC2626', benefits: [''], detailedBenefits: { discount: '0%', cashback: '0%', priority: 'No', promotions: 'Basic', limitBonus: '0%', accessLevel: 'Basic' }, sortOrder: ranks?.length || 0 }); }
  ...
  const handleSaveRank = async () => {
    ...
    try {
      ...
      toast({ title: 'Rank Synchronized', description: `${rankFormData.name} tier updated.` });
      setIsRankModalOpen(false);
    } catch (e: any) { toast({ variant: 'destructive', title: 'Save Failed', description: e.message }); } finally { setSaving(false); }
  };

  const seedDefaults = async () => {
    if (!confirm('Deploy system default ranks?')) return;
    setSaving(true);
    try { for (const rank of DEFAULT_RANKS) { await setDoc(doc(db, 'ranks', rank.id), rank); } toast({ title: 'Defaults Deployed' }); } catch (e: any) { toast({ variant: 'destructive', title: 'Seed Failed', description: e.message }); } finally { setSaving(false); }
  };
