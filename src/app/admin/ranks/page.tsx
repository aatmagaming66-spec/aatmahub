...
export default function RankManagementPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Rank Synchronized', description: `${formData.name} tier is now active.` });
  ...
  const seedDefaults = async () => {
    if (!confirm('Deploy system default ranks to database?')) return;
    setSaving(true);
    try {
      for (const rank of DEFAULT_RANKS) {
        await setDoc(doc(db, 'ranks', rank.id), rank);
      }
      toast({ title: 'System Default Deployed' });
    } catch (e: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-headline font-black tracking-tighter uppercase">Rank Registry</h1>
          <p className="text-[10px] text-muted-foreground uppercase tracking-[0.3em] font-black opacity-60">Tier & Benefit Protocol</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-dashed border-border rounded-[2rem] p-20 text-center">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-40">No Ranks Defined in Registry</p>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[8px] text-muted-foreground font-black uppercase tracking-widest">₹{rank.threshold.toLocaleString()} Requirement</p>
                      </div>
                   </div>
        ...
        <DialogContent className="bg-card border-border rounded-3xl p-8 max-w-xl">
          ...
          <div className="grid grid-cols-2 gap-6 py-6 overflow-y-auto max-h-[60vh] no-scrollbar">
            <div className="space-y-2">
              <Label className="text-[9px] font-black uppercase">Rank ID</Label>
            ...
            <div className="space-y-2">
              <Label className="text-[9px] font-black uppercase">Rank Name</Label>
            ...
            <div className="space-y-2">
              <Label className="text-[9px] font-black uppercase">Threshold (₹)</Label>
            ...
            <div className="space-y-2">
              <Label className="text-[9px] font-black uppercase">Discount (%)</Label>
            ...
            <div className="space-y-2">
              <Label className="text-[9px] font-black uppercase">Accent Color</Label>
            ...
            <div className="space-y-2">
              <Label className="text-[9px] font-black uppercase">Display Order</Label>
          ...
          <DialogFooter>
            <Button onClick={handleSave} disabled={saving} className="w-full bg-primary h-14 rounded-2xl font-black uppercase text-[11px] tracking-widest">
              {saving ? <Loader2 className="animate-spin" /> : "Commit Rank Registry"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
