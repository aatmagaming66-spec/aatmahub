...
export default function SystemSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Settings Saved", description: "General system settings updated." });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-headline font-black tracking-tighter uppercase text-white">General Settings</h1>
          <p className="text-[10px] text-muted-foreground uppercase tracking-[0.3em] font-black opacity-60">Manage Site-wide Configurations</p>
        </div>
        <Button onClick={handleSave} disabled={saving} className="bg-primary h-12 px-8 rounded-none font-black uppercase text-[10px] tracking-widest shadow-xl shadow-primary/20 gap-2">
           {saving ? <Loader2 className="animate-spin" /> : <><Zap size={14} /> Save Changes</>}
        </Button>
      </header>

      <div className="grid lg:grid-cols-2 gap-8">
        <div className="grid grid-cols-2 gap-4">
          <HealthCard icon={Database} label="Database Status" status={health.firestore} color="text-blue-400" />
          <HealthCard icon={ShieldCheck} label="Auth Status" status="operational" color="text-green-400" />
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <Card className="bg-card border-border rounded-none overflow-hidden shadow-2xl">
          <CardHeader className="p-8 border-b border-border bg-black/20">
            <CardTitle className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-white"><Globe className="h-4 w-4 text-primary" /> Maintenance & Branding</CardTitle>
          </CardHeader>
          <CardContent className="p-8 space-y-6">
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-none border border-white/5">
              <div className="space-y-0.5"><Label className="text-xs font-black uppercase">Maintenance Mode</Label><p className="text-[8px] text-muted-foreground uppercase font-black">Turn off public access to the site</p></div>
            ...
            <div className="space-y-2"><Label className="text-[9px] font-black uppercase text-muted-foreground">Website Name</Label><Input value={settings.siteBranding} onChange={(e) => setSettings({...settings, siteBranding: e.target.value})} className="bg-black/50 border-border h-12 rounded-none font-bold" /></div>
          </CardContent>
        </Card>

        <Card className="bg-card border-border rounded-none overflow-hidden shadow-2xl">
          <CardHeader className="p-8 border-b border-border bg-black/20">
            <CardTitle className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-white"><MessageCircle className="h-4 w-4 text-accent" /> Support Information</CardTitle>
          </CardHeader>
          <CardContent className="p-8 space-y-6">
      ...
      <Card className="bg-card border-border rounded-none p-8 shadow-2xl">
        <div className="flex items-center justify-between mb-8">
           <div className="flex items-center gap-3"><History className="h-5 w-5 text-primary" /><h3 className="text-xs font-black uppercase tracking-widest text-white">System Activity Logs</h3></div>
           <span className="text-[8px] font-black bg-primary/10 text-primary px-3 py-1 rounded-full uppercase">Recent Events</span>
        </div>
