...
import { Plus, Edit2, Trash2, Loader2, Search, Share2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import Image from 'next/image';

const SocialCard = memo(function SocialCard({ item, onEdit, onDelete }: any) {
  return (
      ...
      <CardContent className="p-6 space-y-6">
        <div className="flex items-center gap-4">
          <div className="h-16 w-16 bg-black/40 rounded-2xl flex items-center justify-center border border-white/5 shadow-inner overflow-hidden relative">
            {item.icon ? <Image src={item.icon} alt={item.name} fill className="object-cover" /> : <Share2 size={24} className="text-accent" />}
          </div>
...
export default function SocialManagementPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'social_services', id), { ...formData, id, updatedAt: new Date().toISOString() }, { merge: true });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-headline font-black tracking-tighter uppercase">Social Hub</h1>
          <p className="text-[10px] text-muted-foreground uppercase tracking-[0.3em] font-black opacity-60">Growth Service Distribution</p>
        </div>
      ...
      <div className="relative group">
        ...
        <Input 
          placeholder="Search social services..." 
          value={searchQuery}
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <SocialCard key={item.id} item={item} onEdit={handleOpenModal} onDelete={handleDelete} />
          ))}
        </div>
      )}
