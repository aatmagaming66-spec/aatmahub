  ...
  const handleDeposit = async () => {
    ...
    try {
      if (paymentMethod === 'phonepe') {
        ...
        if (!res.ok) throw new Error(text || 'Gateway initiation failure.');
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-2">
          {['100', '500', '1000', '2000'].map((val) => (
            <Button key={val} variant="outline" onClick={() => setAmount(val)} className="bg-card border-border h-12 rounded-none text-[10px] font-black uppercase hover:border-primary hover:text-primary">+₹{val}</Button>
          ))}
        </div>
        ...
        <div className="space-y-6 pt-4">
          ...
          <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="grid gap-3">
            <Label htmlFor="phonepe" className={`flex items-center justify-between p-5 rounded-none border transition-all cursor-pointer bg-card shadow-xl ${paymentMethod === 'phonepe' ? 'border-primary ring-1 ring-primary/20' : 'border-border'}`}>
              <div className="flex items-center gap-4">
                <div className={`h-10 w-10 flex items-center justify-center ${paymentMethod === 'phonepe' ? 'bg-primary/20' : 'bg-white/5'}`}><Smartphone className={paymentMethod === 'phonepe' ? 'text-primary' : 'text-muted-foreground'} /></div>
            ...
            <Label htmlFor="razorpay" className={`flex items-center justify-between p-5 rounded-none border transition-all cursor-pointer bg-card shadow-xl ${paymentMethod === 'razorpay' ? 'border-primary ring-1 ring-primary/20' : 'border-border'}`}>
              <div className="flex items-center gap-4">
                <div className={`h-10 w-10 flex items-center justify-center ${paymentMethod === 'razorpay' ? 'bg-primary/20' : 'bg-white/5'}`}><CreditCard className={paymentMethod === 'razorpay' ? 'text-primary' : 'text-muted-foreground'} /></div>
        ...
        <Button className="w-full h-16 bg-primary hover:bg-secondary text-sm font-black uppercase tracking-[0.2em] shadow-2xl shadow-primary/20 rounded-none transition-all" onClick={handleDeposit} disabled={loading}>{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : `Request ₹${amount || '0'} Deposit`}</Button>
