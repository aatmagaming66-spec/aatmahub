      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <Skeleton key={i} className="aspect-[3/4] w-full rounded-none bg-white/5" />
          ))}
        </div>
      ) : items.length === 0 ? (
            ...
            <div key={item.id} className="bg-card border border-border rounded-none overflow-hidden flex flex-col p-4 shadow-xl">
              ...
              <div className="text-center space-y-4 mt-auto">
                ...
                <Button 
                  ...
                  className="w-full h-10 bg-accent hover:bg-accent/90 rounded-none text-[9px] font-black uppercase tracking-widest gap-2 shadow-xl"
                >
