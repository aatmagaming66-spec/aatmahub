...
export function RankAvatar({ fallback, rank = 'Warrior', className, size = 'md' }: RankAvatarProps) {
  ...
  const getRankStyles = () => {
    if (r.includes('immortal')) return "border-yellow-500 shadow-[0_0_25px_rgba(234,179,8,0.7)] animate-pulse";
    if (r.includes('glory')) return "border-red-600 shadow-[0_0_20px_rgba(220,38,38,0.6)]";
    if (r.includes('honor')) return "border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.5)]";
    if (r.includes('mythic')) return "border-red-500 shadow-[0_0_12px_rgba(239,68,68,0.5)]";
    if (r.includes('legend')) return "border-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.4)]";
    if (r.includes('epic')) return "border-pink-500 shadow-[0_0_10px_rgba(236,72,153,0.3)]";
    if (r.includes('grandmaster')) return "border-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.2)]";
    if (r.includes('master')) return "border-blue-500 shadow-[0_0_6px_rgba(59,130,246,0.2)]";
    if (r.includes('elite')) return "border-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.2)]";
    ...
    <div className={cn("relative inline-block shrink-0", className)}>
      ...
      {r.includes('immortal') && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10 animate-bounce">
          <Crown className="h-5 w-5 text-yellow-500 fill-yellow-500 filter drop-shadow-[0_0_8px_rgba(234,179,8,1)]" />
        </div>
      )}
