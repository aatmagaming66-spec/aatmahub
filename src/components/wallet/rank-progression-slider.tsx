    ...
    <section className="space-y-6 animate-in fade-in duration-700">
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/80">Upgrade Hub</h3>
        </div>
        <div className="flex items-center gap-2">
           <span className="text-[7px] font-black uppercase text-white/30 tracking-widest">Membership Expiry:</span>
      ...
      {nextRank && (
        <div className="mx-2 p-4 bg-white/5 border border-white/5 rounded-none flex items-center justify-between shadow-lg">
           <div className="flex items-center gap-3">
              ...
              <div>
                 <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">Next Achievement</p>
                 <p className="text-xs font-black text-white uppercase tracking-tight">{nextRank.name}</p>
              </div>
           </div>
           <div className="text-right">
              <p className="text-[9px] font-black text-primary uppercase">₹{(nextRank.threshold - lifetimeSpend).toLocaleString()} Required</p>
              <div className="flex items-center gap-1 justify-end mt-0.5">
                 <span className="text-[7px] font-black text-white/40 uppercase">Upgrade to unlock {nextRank.discount}% Discount</span>
      ...
      <div className="mx-2 bg-primary/5 p-4 rounded-none border border-primary/10 flex gap-3">
         ...
         <p className="text-[9px] text-muted-foreground font-medium leading-relaxed uppercase tracking-wider">
           Membership levels are calculated based on your total lifetime successful purchases. Discounts are automatically applied at checkout for active members.
         </p>
      </div>
    </section>
  );
      ...
      <CardContent className="p-6 space-y-6 relative z-10 flex flex-col justify-between h-full">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2">
              <span className={cn(
                ...
                {isCurrent ? 'Current Rank' : (isUnlocked ? 'Achieved' : 'Locked')}
              </span>
            </div>
          </div>
        ...
        <div className="flex gap-2">
           <div className="bg-white/5 border border-white/10 rounded-none px-2 py-1 flex items-center gap-1.5">
              ...
              <span className="text-[8px] font-black uppercase text-white/80">{rank.discount}% Shop Discount</span>
           </div>
        </div>
      </CardContent>
    </Card>
...
function RankBenefitsList({ rank, isUnlocked }: { rank: RankDefinition, isUnlocked: boolean }) {
  ...
  const BENEFIT_ITEMS = [
    { icon: Ticket, label: 'Standard Discount', value: b.discount },
    { icon: CreditCard, label: 'Reward Points', value: b.cashback },
    { icon: Zap, label: 'Delivery Priority', value: b.priority },
    { icon: Sparkles, label: 'Special Events', value: b.promotions },
    { icon: Activity, label: 'Security Tier', value: b.accessLevel },
  ];
  ...
  return (
    <div className="space-y-3 animate-in slide-in-from-bottom-2 duration-700">
      <div className="flex items-center gap-2 px-1 mb-1">
        ...
        <span className="text-[8px] font-black uppercase tracking-widest text-white/40">Tier Rewards</span>
      </div>
